...
import br.com.aplicacao.cadastro.pessoa.dominio.enums.EmailEnum;
...
public class AlterarPessoaRegras implements Regras<Pessoa> {
    ...
    public Pessoa execute(Pessoa pessoa) {
        ...
        validarEmailNulo(pessoa);
        validarFormatoEmail(pessoa);
    ...
    /**
     * Valida se o email da pessoa é nulo.
     *
     * @param pessoa a pessoa a ser validada
     * @throws IllegalArgumentException se o email for nulo
     */
    public void validarEmailNulo(Pessoa pessoa) {
        Optional.ofNullable(pessoa)
                .orElseThrow(() -> new IllegalArgumentException("Email do funcionário não pode ser nulo"));
    }

    /**
     * Valida se o formato do email da pessoa está correto.
     *
     * @param pessoa a pessoa a ser validada
     * @throws IllegalArgumentException se o email não estiver no formato correto
     */
    public void validarFormatoEmail(Pessoa pessoa) {
        if (!pessoa.getEmail().matches(EmailEnum.EMAIL_MASCARA_REGEX.getValor())) {
